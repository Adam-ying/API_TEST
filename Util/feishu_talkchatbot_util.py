...
class FeiShutalkChatbot(object):
    def __init__(self, webhook, secret=None, pc_slide=False, fail_notice=False):
        '''
        飞书机器人初始化
        :param webhook: 飞书群自定义机器人webhook
        :param secret:  机器人安全设置页面勾选“加签”时需要传入的密钥
        :param pc_slide:  消息链接打开方式，默认False为浏览器打开，设置为True时PC端侧边栏打开
    ...
    def send_text_post(self, msg, open_id=[]):
        """
        消息类型为富文本类型
        :param msg:  消息内容
        :param open_id:
        msg格式:
        {
            "title": "[API]项目: Content_Intelligence_Dam",
            "content": [
                [{
                    "tag": "text",
                    "text": "Content_Intelligence_Dam :"
                }],
                [{
                    "tag": "text",
                    "text": "用例名称："
        ...
        try:
            ...
            try:
                ...
                if self.fail_notice and result.get('errcode', True):
                    ...
                    res = requests.post(self.webhook, headers=self.headers, data=json.dumps(error_data), verify=False)
                    # print(res.json())
                return result


if __name__ == '__main__':
    feishu = FeiShutalkChatbot(webhook_url)
    feishu.fail_notice = False
    data = {
        "title": "富文本信息",
        "content": [
            [{"tag": "text", "text": "富文本信息: "},
